...
# Set print_data for additional information on each iteration.
# Uses a kernel k for computations, returns a mistake counter for each example
def dual_perceptron(data, k, T, print_data):
	a = np.zeros(data.shape[0], dtype='float64')
	...
	for i in range(len(a)):
		for j in range(len(a)):
			mf[i][j] = y_data[j] * k(x_data[j], x_data[i])
